...
<div class="row">
	<div class="large-6 columns large-offset-3">
		...
		<form method="post" action="{{ url('hubly_signup') }}">
			...
			<input id="sign_up_email" type="email" name="email" placeholder="Your Email" {% if response.code == 2 or response.code == 4 %}class="error"{% endif %}{% if args.email %}value="{{ args.email }}" {% endif %}/>
			<label for="sign_up_email_confirmation" class="hide">Email address confirmation</label>
			<input id="sign_up_email_confirmation" type="email" name="email_confirmation" placeholder="Confirm Email"{% if response.code == 3 or response.code == 6 or response.code == 4 %}class="error"{% endif %}{% if args.email %}value="{{ args.email }}" {% endif %}/>
